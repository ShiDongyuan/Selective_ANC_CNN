...
class SoundGenerator:
    ...
    def _construct_(self):
        ...
        #save_specgram(noise, self.fs, fileName=os.path.splitext(filePath)[0])
        #plot_specgram(noise, fs)
        return f_star[0], f_end[0], filename #filePath
    ...
    def _construct_A(self):
        ...
        #save_specgram(noise, self.fs, fileName=os.path.splitext(filePath)[0])
        #plot_specgram(noise, fs)
        # plot_specgram(noise,self.fs,os.path.splitext(filePath)[0]+".png")
        return f_star[0], f_end[0], filename #filePath  
...
#-----------------------------------------------------------------
F_LEVEL = [[20, 1200], [1000, 3000], [2700, 5700], [5600, 7980]]
# F_LEVEL = [[20, 550], [450, 1200], [1000, 3700], [3500, 7980]]
# F_LEVEL = [[20, 1550], [1000, 2200], [2300, 4700], [4500, 7980]]
class ClassID_Calculator:
...
if __name__ == "__main__":
    ...
    Generator     = SoundGenerator(fs=16000, folder = "testing_data_v1")
    datasheet     = DatasetSheet("testing_data_v1","Index.csv")
    ID_calculator = ClassID_Calculator(F_LEVEL)
